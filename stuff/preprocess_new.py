...
def Ding_abstract(label,bigram,trigram,types='title'):
    ...
    title=title.replace(['\d+\S\d+','\d+'], 'xxx', regex=True)#becareful of using inplace
...
def split(data, label, bigram, trigram):
    ...
    for key, value in data.items():
        try:
            data_clean.append([convert(i, bigram, trigram, True) for i in set(value)])
            date.append(key)
            lab.append(label[key])
        except (KeyError, TypeError) as e:
            continue
    cc = pd.DataFrame({'date': pd.Series(date, index=date)})
    ll = pd.DataFrame({"label": pd.Series(lab, index=date, dtype='int32')})

    d = pd.DataFrame(data_clean, index=date,
                     columns=range(0, len(max(data_clean, key=len))))  # put the convert words into a new Dataframe
    print(d, ll)
    final = pd.merge(ll, d, left_index=True, right_index=True)  # merge two list
    finall = pd.merge(cc, final, left_index=True, right_index=True)
    finall = finall.dropna(axis=0, thresh=3)
    print(finall.shape)
    return finall
