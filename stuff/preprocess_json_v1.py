...
def Ding_abstract(label, bigram, trigram, path='/Users/maobu/Dropbox/stock/data/ding/'):
    ...
    with open('/Users/maobu/Documents/datas/Yunke_embedding_v2/GOOG.json',
              "r") as data:  # title+ abstract + article
        title = pd.DataFrame(json.loads(line, strict=False) for line in data).set_index('date')
    title.index = title.index.to_series().apply(lambda x: (datetime.strptime(x, '%Y%m%d')).strftime('%Y-%m-%d'))
    title = title.sort_index()
    title = title.replace(['UPDATE\s\d-', "'s"], '', regex=True)  # becareful of using inplace
    title = title.replace(['S&P; 500','S&P'], 's_p', regex=True)  # becareful of using inplace
    title = title.replace(["U.S.",'US'], 'u_s', regex=True)  # becareful of using inplace
    ...
    for i in types:
        title[i] = title[i].apply(convert, args=(bigram, trigram, True))
    print(title.shape)
    # title = title.replace('^\s+$', np.nan, regex=True)
    print(title.shape)
    ...
    '''
    train = title  # from the beginning 2006-10-20 plus 30 days after, just in case
    validate = title  # plus 30 days before and after, just in case
    test = title  # plus 30 days before and after, just in case
...
def main():
    ...
    label_one = pd.read_pickle("label_one_new_GOOG.pickle")['2014-05-01':]  # ['2006-11-20':'2013-11-21']
