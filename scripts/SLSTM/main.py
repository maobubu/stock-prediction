...
def prepare_data(sequence, sequence_d1, sequence_d2, labels, options, maxlen=None, max_word=100):
    ...
    ##TODO for x
    x = numpy.zeros((maxlen_x, n_samples, max_sentence, max_sequence)).astype('int64')
    x_mask_sentence = numpy.zeros((maxlen_x, n_samples, max_sentence)).astype('float32')
    x_mask_news = numpy.zeros((maxlen_x, n_samples)).astype('float32')
    ##TODO for x_d1
    x_d1 = numpy.zeros((day1, maxlen_xd1, n_samples, max_sentence_d1, max_sequence_d1)).astype('int64')
    x_d1_mask_sentence = numpy.zeros((day1, maxlen_xd1, n_samples, max_sentence_d1)).astype('float32')
    x_d1_mask_news = numpy.zeros((day1, maxlen_xd1, n_samples)).astype('float32')
    ##TODO for x_d2
    x_d2 = numpy.zeros((day2, maxlen_xd2, n_samples, max_sentence_d2, max_sequence_d2)).astype('int64')
    x_d2_mask_sentence = numpy.zeros((day2, maxlen_xd2, n_samples, max_sentence_d2)).astype('float32')
    x_d2_mask_news = numpy.zeros((day2, maxlen_xd2, n_samples)).astype('float32')
        ...
        for idx, ss in enumerate(i):  # time step
            for m, n in enumerate(ss):
                if len(n) < max_sequence:
                    x[idx, index, m, :len(n)] = n
                else:
                    x[idx, index, m, :max_sequence] = n[:max_sequence]
                x_mask_sentence[idx, index, m] = 1.
            x_mask_news[idx, index] = 1.
        ##TODO x_d1
        for jj, day in enumerate(j):
            for idx, ss in enumerate(day):
                for m, n in enumerate(ss):
                    if len(n) < max_sequence_d1:
                        x_d1[jj, idx, index, m, :len(n)] = n
                    else:
                        x_d1[jj, idx, index, m, :max_sequence_d1] = n[:max_sequence_d1]
                    x_d1_mask_sentence[jj, idx, index, m] = 1.
                x_d1_mask_news[jj, idx, index] = 1.
        ##TODO x_d2
        for jj, day in enumerate(k):
            for idx, ss in enumerate(day):
                for m, n in enumerate(ss):
                    if len(n) < max_sequence_d2:
                        x_d2[jj, idx, index, m, :len(n)] = n
                    else:
                        x_d2[jj, idx, index, m, :max_sequence_d2] = n[:max_sequence_d2]
                    x_d2_mask_sentence[jj, idx, index, m] = 1.
                x_d2_mask_news[jj, idx, index] = 1.
...
def day_lstm(input, sequence_mask, x_mask_sentence, x_mask, keep_prob, is_training, options):
    # input is day*time_step*batch*sentence*sequence_step*embedding, 4*40*32*10*13*100
    # sequence_mask shape is time_step,batch,sentence*sequence_step, 4*40*32*10*13
    # x_mask_sentence shape is day,time_step,batch,sentence 4*40*32*10
    ...
    def body(i, x, mask, mask_1, mask_2, out_):
        emb = sentence_lstm(x[i], mask[i], mask_1[i], keep_prob, is_training, options)
        # emb shape is news,batch,hidden units 40*32*600
        ...
        att = attention_v2(tf.transpose(emb, [1, 0, 2]), tf.transpose(mask_2[i], [1, 0]),
                           name='news_attention', keep=keep_prob,is_training=is_training)  # already masked after attention

        ##TODO average news
        # att = tf.reduce_sum(emb, 0) / tf.expand_dims(tf.reduce_sum(mask_2[i], 0) + 1e-8,1)
        # shape is 32*600

        # att shape is day*batch,hidden units 4*32*600
        out_ = out_.write(i, att)

        return i + 1, x, mask, mask_1, mask_2, out_

    _, _, _, _, _, result = tf.while_loop(cond, body, [t, input, sequence_mask, x_mask_sentence, x_mask, output_list])
    result = result.stack()  # result shape is day*time_step,batch,hidden units 4*40*32*600

    return result


def sentence_lstm(input, sequence_mask, x_mask, keep_prob, is_training, options):
    # input news,batch,sentence,sequence,embedding 40*32*10*13*100
    # sequence_mask shape is news,batch,sentence,sequence_step, 40*32*10*13
    # x_mask shape is news, batch, sentence, 40*32*10
    ...
    def body(i, x, mask, mask_1, out_):
        ##TODO word evel Bilstm
        out = sequence_lstm(tf.transpose(x[i], perm=[1, 0, 2, 3]), tf.transpose(mask[i], perm=[1, 0, 2]), keep_prob,
                            is_training, options)
        # out shape is sentence * batch * 2*lstm_unit (concate) 10*32*600
        ##TODO sentence level Bilstm
        out = bilstm_filter(out, tf.transpose(mask_1[i], [1, 0]), keep_prob,
                            prefix='sentence_encode', dim=options['dim'],
                            is_training=is_training)  # output shape: sequence_step,batch,2*lstm_unit(concate) 10*32*600

        out = tf.concat(out, 2) * tf.expand_dims(tf.transpose(mask_1[i], [1, 0]), -1)  # mask after sentence bilstm
        # mask the output 10*32*600

        ##TODO sentence level attention
        att = attention_v2(tf.transpose(out, [1, 0, 2]), mask_1[i],
                           name='sentence_attention', keep=keep_prob,is_training=is_training)
        # attention shape 32*600

        ##TODO average sentence
        # att = tf.reduce_sum(out, 0) / tf.expand_dims(tf.reduce_sum(mask_1[i], 1) + 1e-8, 1)
        # shape is 32*600

        out_ = out_.write(i, att)

        return i + 1, x, mask, mask_1, out_
    ...
    # result shape is 40*32*600
    return result


def sequence_lstm(input, sequence_mask, keep_prob, is_training, options):
    # input sentence,batch,sequence_step,embedding, 10*32*13*100
    # sequence_mask shape is sentence,batch,sequence_step, 10*32*13
    time_step = tf.shape(input)[0]
    # time_step = input.get _shape().as_list()[0]
    output_list = tf.TensorArray(dtype=tf.float32, size=time_step)
    t = tf.constant(0, dtype=tf.int32)
...
def build_model(embedding, options):
    ...
    with tf.device('/gpu:0'):
        ...
        ##TODO word level and sentence level LSTM
        emb = sentence_lstm(emb, sequence_mask, x_mask_sentence, keep_prob, is_training,
                            options)  # emb shape is 40*32*600
        emb = emb * tf.expand_dims(x_mask, -1)  # mask before attention

        ##TODO news level attention
        att = attention_v1(tf.transpose(emb, [1, 0, 2]), tf.transpose(x_mask, [1, 0]), name='news_attention',
        ...
        #att_day1 = day_lstm(emb_d1, sequence_d1_mask, x_d1_mask_sentence, x_d1_mask, keep_prob, is_training, options)
        '''if options['use_dropout']:
            att_day1 = tf.nn.dropout(att_day1, keep_prob)'''
        # TODO bilstm layers
        # Change the time step and batch
    with tf.device('/gpu:0'):
        #att_day2 = day_lstm(emb_d2, sequence_d2_mask, x_d2_mask_sentence, x_d2_mask, keep_prob, is_training, options)
        '''if options['use_dropout']:
            att_day2 = tf.nn.dropout(att_day2, keep_prob)'''
        ...
        if options['last_layer'] == 'LSTM':
            ...
            #logit = fflayer_2D(options, att, name='ff', activation_function=tf.nn.relu, nin=2 * options['dim'],
