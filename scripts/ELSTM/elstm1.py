...
# New multiprocess version


def get_elmo_batch(batch_data, max_seq_length, embedding_file, day_flag=False):
    ...
    if day_flag is False:
        one_sample = []
        for news in range(len(batch_data)):
            if int(batch_data[news]) == -1:
                elmo_news = zero_news
            else:
                with h5py.File(embedding_file, 'r') as fin:
                    elmo_news = np.average(fin[str(batch_data[news])], axis=0).tolist()
        ...
        return np.array(one_sample)
    # deal with batch with days
    else:
        one_sample = []
        for days in range(len(batch_data)):
            one_day = []
            for news in range(len(batch_data[days])):
                if int(batch_data[days][news]) == -1:
                    elmo_news = zero_news
                else:
                    with h5py.File(embedding_file, 'r') as fin:
                        elmo_news = np.average(fin[str(batch_data[days][news])], axis=0).tolist()
                while len(elmo_news) < max_seq_length:
                    elmo_news.append(zero_word)
...
def get_elmo(data, length, embedding_f, day):
    process_pool = multiprocessing.Pool()

    results = process_pool.map(
        func=functools.partial(get_elmo_batch, max_seq_length=length,
                               embedding_file=embedding_f, day_flag=day),
...
def days(emb, sequence_mask, news_mask, keep_prob, is_training, options, elmo):
    ...
    word_level_inputs = tf.concat([word_level_inputs, elmo], axis=2)
...
def predict_pro_acc(sess, cost, prepare_data, model_options, iterator, maxlen, correct_pred, pred, summary, eidx,
    ...
    for x_sent, x_d1_sent, x_d2_sent, y_sent, y_tech, elmo_d0, elmo_d1, elmo_d2 in iterator:
        ...
        max_sequence_d1, max_sequence_d2 = prepare_data(
            ...
            maxlen=maxlen)
        if validate is True:
            elmo_d0_embedding = get_elmo(elmo_d0, max_sequence_d0, model_options['validate_elmo'], False)
            elmo_d1_embedding = get_elmo(elmo_d1, max_sequence_d1, model_options['validate_elmo'], True)
            elmo_d2_embedding = get_elmo(elmo_d2, max_sequence_d2, model_options['validate_elmo'], True)
        else:
            elmo_d0_embedding = get_elmo(elmo_d0, max_sequence_d0, model_options['test_elmo'], False)
            elmo_d1_embedding = get_elmo(elmo_d1, max_sequence_d1, model_options['test_elmo'], True)
            elmo_d2_embedding = get_elmo(elmo_d2, max_sequence_d2, model_options['test_elmo'], True)
        ...
        for eidx in range(max_epochs):
            ...
            for x, x_d1, x_d2, y, y_tech, elmo_d0, elmo_d1, elmo_d2 in train:
                ...
                data_x, data_x_mask, data_x_d1, data_x_d1_mask, data_x_d2, data_x_d2_mask, data_y, final_mask,\
                    ...
                    model_options,
                    maxlen=maxlen)
                elmo_d0_embedding = get_elmo(elmo_d0, max_sequence_d0, model_options['train_elmo'], False)
                elmo_d1_embedding = get_elmo(elmo_d1, max_sequence_d1, model_options['train_elmo'], True)
                elmo_d2_embedding = get_elmo(elmo_d2, max_sequence_d2, model_options['train_elmo'], True)
