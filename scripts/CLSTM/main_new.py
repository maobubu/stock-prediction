...
def days(emb, sequence_mask, news_mask, keep_prob, is_training, options):
    ...
    news_level_input = tf.reshape(word_level_output, [batch * day, new_s,options['dim_word']])  # (32*3)*40*600 #2 * options['dim']
    news_level_input = news_level_input * tf.expand_dims(news_level_mask, -1)  # mask before attention

    ##TODO news level CNN
    news_level_output = cnn_sent(news_level_input, options, name='news_cnn')
    ...
    # shape is (32*3)*600
    day_level_output = tf.reshape(news_level_output, [batch, day, 3*options['CNN_filter']])# (32*3)*600
...
def news(emb, sequence_mask, news_mask, keep_prob, is_training, options):
    ...
    if options['use_dropout']:
        word_level_output = layers.dropout(word_level_output, keep_prob=keep_prob, is_training=is_training, seed=None)
    news_level_input = tf.reshape(word_level_output, [batch, new_s, options['dim_word']])  # 32*40*600  #2 * options['dim']
...
def cnn_sent(input, options, name='news_cnn'):
    with tf.variable_scope(name_or_scope=name, reuse=tf.AUTO_REUSE):
    ...
    with tf.device('/gpu:1'):
        ...
        with tf.name_scope('final'):
            ...
            if options['last_layer'] == 'LSTM':
                ...
                att = attention_v2(tf.concat(final, 2), final_mask, name='day_attention', keep=keep_prob, r=4,
            ...
            if options['last_layer'] == 'CNN':
                with tf.variable_scope(name_or_scope='day_cnn', reuse=tf.AUTO_REUSE):
            ...
            for x, x_d1, x_d2, y in train:
                ...
                if numpy.mod(uidx, validFreq) == 0:
                    ...
                    valid_acc, valid_loss = predict_pro_acc(sess, cost, prepare_data, model_options, valid, maxlen,
                                                            correct_pred, pred, summary, eidx, is_training, train_op,
                                                            loss_plot,
