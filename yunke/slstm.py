            ...
            for idx, ss in enumerate(day):
                for m, n in enumerate(ss):
                    if len(n) < max_sequence_d2:
                        x_d2[index, jj, idx, m, :len(n)] = n
                    else:
                        x_d2[index, jj, idx, m, :max_sequence_d2] = n[:max_sequence_d2]
                    x_d2_mask_sentence[index, jj, idx, m] = 1.
                x_d2_mask_news[index, jj, idx] = 1.
...
def news(emb, sequence_mask, sentence_mask, news_mask, keep_prob, is_training, options):
    ...
    ##TODO prepare sentence level input
    sentence_level_input = tf.reshape(word_level_output,
                                      [batch * new_s, sentence, 2 * options['dim']])  # (32*40)*20*600
...
def attention_v2(input, mask, name='attention', nin=600, keep=1.0, r=10, is_training=True):
    ...
    with tf.variable_scope(name_or_scope=name, reuse=tf.AUTO_REUSE):
        ...
        '''
    return outputs  # result shape is batch, hidden_unit (32*40)*600
...
def bilstm_filter(input, mask, keep_prob, prefix='lstm', dim=300, is_training=True):
    with tf.variable_scope(name_or_scope=prefix, reuse=tf.AUTO_REUSE):
        sequence = tf.cast(tf.reduce_sum(mask, 1), tf.int32)
        lstm_fw_cell = rnn.LSTMBlockCell(dim, forget_bias=0.2)
        # back directions
        lstm_bw_cell = rnn.LSTMBlockCell(dim, forget_bias=0.2)
        keep_rate = tf.cond(is_training is not False and keep_prob < 1, lambda: 0.8, lambda: 1.0)
        cell_dp_fw = rnn.DropoutWrapper(cell=lstm_fw_cell, output_keep_prob=keep_rate)
...
def build_model(embedding, options):
    ...
    with tf.device('/gpu:1'):
        ...
        with tf.name_scope('final'):
            ...
            correctpreds = tf.equal(tf.arg_max(preds, 1), tf.arg_max(labels, 1))
            accuracy = tf.reduce_mean(tf.cast(correctpreds, dtype=tf.float32))
    ...
    return is_training, cost, x, x_mask, y, n_timesteps, preds, summary, accuracy


def predict_pro_acc(sess, cost, prepare_data, model_options, iterator, maxlen, correct_pred, pred, summary, eidx,
                    is_training, plot, writer=None, validate=False):
    ...
    for x_sent, x_d1_sent, x_d2_sent, y_sent in iterator:
        ...
        loss, result, preds, accuracy = sess.run([cost, correct_pred, pred, accuracy],
    ...
    return final_acc, final_loss, acc
...
def train(
    ...
    is_training, cost, x, x_mask, y, n_timesteps, pred, summary, accu = build_model(embedding, model_options)
    with tf.variable_scope('train'):
        ...
        ab_cost = cost
        lossL2 = tf.add_n([tf.nn.l2_loss(v) for v in tvars if ('embeddings:0' or 'bias') not in v.name]) * 0.0001  #
        cost = cost + lossL2
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=False)) as sess:
        ...
        #learning_rate = tf.train.exponential_decay(model_options['lrate'], 1, 100000, 0.96, staircase=True)
        #assign_lr(sess, learning_rate)
        for eidx in range(max_epochs):
            learning_rate = tf.train.exponential_decay(model_options['lrate'], eidx, 3, 0.8, staircase=True)
            assign_lr(sess, learning_rate)
            n_samples = 0
            training_cost = 0
            for x, x_d1, x_d2, y in train:
                ...
                if x is None:
                    ...
                    continue
                ud_start = time.time()
                _, loss, accuracy = sess.run([train_op, op_loss, accu],
                                   feed_dict={'input/x:0': data_x, 'input/x_mask_sentence:0': data_x_mask_sentence,
                ...
                if numpy.mod(uidx, validFreq) == 0:
                    ...
                    valid_acc, valid_loss, vacc = predict_pro_acc(sess, cost, prepare_data, model_options, valid, maxlen,
                                                            correct_pred, pred, summary, eidx, is_training, loss_plot,
                                                            validate_writer, validate=True)
                    test_acc, test_loss, tacc = predict_pro_acc(sess, cost, prepare_data, model_options, test, maxlen,
                                                          correct_pred, pred, summary, eidx, is_training, loss_plot,
                                                          testing_writer)
                    ...
                    logger.debug('new_vaid_acc:  {0}'.format(vacc))
                    logger.debug('new_test_acc:  {0}'.format(tacc))
                    logger.debug('ab_loss:  {0}'.format(ab_cost))
                    logger.debug('l2 regu:  {0}'.format(lossL2))
            ...
            loss_plot['training_ep'].append(training_cost / math.ceil(n_samples / model_options['batch_size']))
    ...
    with tf.Session() as sess:
        ...
        valid_acc, valid_cost,_ = predict_pro_acc(sess, cost, prepare_data, model_options, valid,
        ...
        test_acc, test_cost,_ = predict_pro_acc(sess, cost, prepare_data, model_options, test,
                                              maxlen, correct_pred, pred, summary, eidx, is_training, None)
