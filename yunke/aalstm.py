...
def attention_v1(input, masks, is_training, name='attention', nin=600, keep=1.0, hidden_dim = 300, actvation_f=tf.nn.relu):
    ...
    with tf.variable_scope(name_or_scope=name, reuse=tf.AUTO_REUSE):
        hidden = tf.layers.dense(input, hidden_dim, activation=actvation_f, use_bias=True,
    ...
    with tf.device('/gpu:1'):
        with tf.variable_scope('news'):
            ...
            news_encode = attention_v1(sentence_output, news_sequence_mask, is_training, name="sequence_attention",
                                       keep=DENSE_KEEP_PROB, hidden_dim=600, actvation_f=tf.nn.tanh)
            news_encode = tf.reshape(news_encode, [batch, news, LSTM_DIMENSION * 2 * 2])
            news_output = attention_v1(news_encode, x_news_mask, is_training, name='news_attention',
                                       keep=DENSE_KEEP_PROB)
        ...
        with tf.name_scope('final'):
            ...
            logit = tf.layers.dense(news_output, 300, activation=tf.nn.relu, use_bias=True,
                                        kernel_initializer=layers.xavier_initializer(uniform=True, seed=None,
                                                                                     dtype=tf.float32),
        ...
        for eidx in range(max_epochs):
            ...
            while data_counter + BATCH_SIZE < len(train_news):
                ...
                if early_stop_counter == 20:
                    logger.info("Finished with " + str(best_epoch_num) + " epoch model with minimum valid cost"
                                + str(best_valid_cost))
                    break
