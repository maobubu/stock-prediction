...
def attention_v2(input, mask, name='attention', nin=600, keep=1.0, r=10, is_training=True):
    ...
    with tf.variable_scope(name_or_scope=name, reuse=tf.AUTO_REUSE):
        ...
        # over_hidden = tf.layers.batch_normalization(over_hidden, training=is_training)
        # over_hidden=tf.nn.tanh(hidden)
    ...
    with tf.device('/gpu:1'):
        ...
        with tf.name_scope('final'):
            ...
            if options['last_layer'] == 'LSTM':
                final = bilstm_filter(final, final_mask, keep_prob, prefix='day_lstm', dim=50,
                ...
                ##TODO take the last
                # att=final[:,-1:]
                logit = tf.layers.dense(att, 100, activation=tf.nn.tanh, use_bias=True,
                                        kernel_initializer=layers.xavier_initializer(uniform=True, seed=None,
                                                                                     dtype=tf.float32),
